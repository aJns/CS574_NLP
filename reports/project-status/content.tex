...
Our approach is to try out extractive methods first. If we can get extractive
methods to work okay on our dataset, we will move on to abstractive methods.
As will be mentioned in Section~\ref{sec:experiments}, we plan to first analyze
the books in our dataset to find out what kind of features should be focused on
in the summary.
...
\begin{table}[hbt]
	\centering
	\caption{Example BLEU 1-gram and 2-gram evaluation for online summaries of
	the Adventures of Huckleberry Finn.}\label{table:bleu_huckfinn}
	\begin{tabular}{l l l }
		\toprule
